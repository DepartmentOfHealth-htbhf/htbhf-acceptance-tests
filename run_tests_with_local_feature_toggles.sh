...
# if this is different, set WEB_UI_DIR to the correct location
# source ./set_feature_toggles.sh

if [[ -z "${WEB_UI_DIR}" ]]; then
  WEB_UI_DIR=../htbhf-applicant-web-ui
fi

export FEATURE_TOGGLES=`cat "${WEB_UI_DIR}"/features.json`
export TAGS=$(cat ${WEB_UI_DIR}/features.json | grep false | sed 's/": false//g' | sed 's/"/ and not @/g' | sed "s/,//g" | sed "s/_ENABLED//g" | tr '\n' ' ')
